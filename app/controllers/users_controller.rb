class UsersController < ApplicationController
	...
	def question_one
		@user = current_user
		if params[:symptom] == "chest_pain" || "shortness_of_breath"
			render "acs.html.erb"
		elsif params[:symptom]== "burning_with_urination"
			render "std.html.erb"
		end
	end
	...
	def acs
		...
		if @final_risk_score >= 15
			@risk_level = "High"
		elsif @final_risk_score >= 10 && @final_risk_score < 15
			@risk_level = "Medium"
		elsif @final_risk_score < 10
			@risk_level = "low"
		end
		...
		# <p> Your estimated total expenditure (includes co-pay and service charge, and remaining deductible, if applicable): <%= @outpatient_services_liability %> </p>
 	# 	<hr>
